<body class="body">
    ...
    <form method="post">
        ...
        foreach ($liste as $key => $value) {
            ...
            echo htmlspecialchars($value['avatar']);
        ?>
            <div class="bclArticle">
                <div>
                    <!-- <img class="img_accueil" src="<?php echo htmlspecialchars($value['avatar']); ?>"> -->
                    <h2 class="article_accueil">
                        <a href="index.php?Sujet=<?php echo htmlspecialchars($value["ID_sujet"]); ?>" class="link-style">
                            <?php echo htmlspecialchars($value['Titre_sujet']); ?>
                        </a>
                    </h2>
                    <div class="author_info">
                        <h3 class="pseudo_accueil"> <?php echo htmlspecialchars($value['pseudo']); ?></h3>
                        <p class="date_accueil"> <?php echo htmlspecialchars($value['Date_creation']); ?></p>
                    </div>
                </div>
                <div>
                    <?php
                    ...
                    if (isset($_SESSION["ID_utilisateur"]) && !empty($_SESSION["ID_utilisateur"])) {
                        if ($_SESSION["ID_utilisateur"] == $value["ID_utilisateur"] || $_SESSION["ID_role"] == 1) { ?>
                            <form method="POST">
                                <input type="hidden" name="SupprSujet" value="<?php echo htmlspecialchars($value["ID_sujet"]); ?>">
